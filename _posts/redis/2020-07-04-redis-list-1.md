...
`LRANGE`命令用于获取列表中由索引值`start`及`stop`指定的范围内的元素，索引值从`0`开始（即`0`为列表中第一个元素），负数值表示相对于列表队尾的位置（如`-1`代表列表中最后一个元素，`-2`代表列表中倒数第二个元素）。`LRANGE`命令返回的元素将包含`start`及`stop`范围内的所由元素，如使用`LRANGE list 0 10`将获取到索引为`0`到`10`的总共11个元素。
