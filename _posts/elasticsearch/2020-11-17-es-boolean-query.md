...
ElasticSearch支持类似于在SQL中使用`AND`、`OR`的运算，称之为布尔查询（Boolean Query），其包括有以下几种类型：
...
在使用布尔查询时，需要将查询的条件写在`bool`查询中，且同个`bool`查询可以有多个不同的条件。例如：
...
该查询将返回所有`age`的值为`20`且`gender`的值不为`"male"`的文档。

下面，我们将一一介绍各个类型的查询。

### `must`查询

当使用`must`时，文档必须符合其中查询条件，例如以下示例：
...
使用`filter`查询时其效果等同于`must`查询，但不同于`must`查询的是，`filter`不参与查询结果的分值计算，它返回的分值始终为0。`filter`的使用场景适合于过滤不需要的文档，但又需要使其不影响最终的顺序。
...
`should`查询与SQL中的`OR`运算不同的一点，`should`查询可以使用`minimum_should_match`参数指定至少需要满足几个条件。例如下面的例子中，查询的结果需要满足两个或两个以上的查询条件：
...
在同一个`bool`语句中若不存在`must`或`filter`时，`minimum_should_match`默认的值为1，即至少要满足其中一个条件；但若有其它`must`或`filter`存在时，`minimum_should_match`默认值为0。例如下面的查询，结果中可能包括有`status`值不为`"active"`的文档。
...
`must_not`查询类似于SQL语句中的`NOT`运算，例如：
